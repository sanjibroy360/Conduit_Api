...
exports.updateArticle = async function (req, res, next) {
                            try {
                                ...
                                if (article.author == req.user.userId) {
                                    ...
                                    var updatedArticle = await Article.findByIdAndUpdate(article.id, req.body, { new: true });
...
exports.deleteArticle = async function (req, res, next) {
                            try {
                                ...
                                if (article && article.author == req.user.userId) {
                                    ...
                                    var updateAllUser = await User.updateMany({ favoritedArticles: { $in: [slug] } }, { $pull: { favoritedArticles: slug } }, {new : true});
                                    var deleteAllComments = await Comment.deleteMany({article : req.params.slug });
